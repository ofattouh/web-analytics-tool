<?php
  ...
  if ( isset($ca_parent) && $ca_parent !== '' ) {
    $res = $ca_parent->sub_account_list_table('last_name','ASC',$currpage,$perpage,$search);
  ...
  // Helpful Email information
  $tooltipText = '<p class="tooltipMeprLoginUnameLabel">'.
  'Note that emails are used only to establish a survey user and are not stored or connected '.
  'with your individual responses. Your responses are not attributed and kept entirely confidential</p>';
...
<div class="mp_wrapper">
  ...
  <div id="mpca-add-sub-user" class="mpca-fat-bottom">
    ...
    <form action="" method="post" id="mpca-add-sub-user-form" class="mpca-hidden">
      ...
      <label>
        <span><?php _e('Existing Participant? (Enter work Email addresss) '.$html_message, 'memberpress-corporate'); ?> </span>
      </label>
      <?php if(MeprUtils::is_mepr_admin()): ?>
        <input value="" type="text" name="userdata[existing_login]" class="mepr_suggest_user" placeholder="<?php _e('Begin typing Email address...', 'memberpress', 'memberpress-corporate') ?>" />
      <?php else: ?>
    ...
    <?php if(!$mepr_options->username_is_email): ?>
      ...
      <label>
        <span><?php _e('New Participant? (Enter work Email address) '.$html_message, 'memberpress-corporate'); ?> </span>
  ...
  <div id="mpca_import_sub_accounts">
    ...
    <form action="" method="post" enctype="multipart/form-data">
      ...
      <?php _e('Send NEW participants the welcome email if they haven\'t received it already for access to the OSIR assessment tool.', 'memberpress-corporate'); ?>
      <br/><br/>
